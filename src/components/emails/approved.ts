...
export async function DescPdf(email: string, role: Role): Promise<Buffer> {
  try {
    ...
    drawTextWithCheck(
      "1. Role Overview",
      { size: subHeaderFontSize, bold: true, color: [0.13, 0.22, 0.39] },
      25,
    );
    ...
    drawParagraph(
      "We are thrilled to welcome you to Invitation Home Rentals. For inquiries, please contact hr@invitationhomesrental.com.",
    );
